...
Effect::Effect(std::string path)
{
  ...
  throwIfFailed(D3DX11CreateEffectFromMemory(
    content.data(), size, 0, s_graphicsDevice->device(), &effect_));
}
  ...
  : Effect(path)
{
  ...
  hasDiffuseTex2D_ =
    effect_->GetVariableByName("g_hasDiffuseTex2D")->AsScalar();
  diffuseTex2D_ =
    effect_->GetVariableByName("g_diffuseTex2D")->AsShaderResource();
  hasDiffuseTexCube_ =
    effect_->GetVariableByName("g_hasDiffuseTexCube")->AsScalar();
  diffuseTexCube =
    effect_->GetVariableByName("g_diffuseTexCube")->AsShaderResource();
...
void
BasicEffect::apply(ID3D11DeviceContext* device_context)
{
  tech_->GetPassByIndex(0)->Apply(0, device_context);
}

void
BasicEffect::vertexShaderBytecode(const void** bytecode, size_t* bytecodeLength)
{
...
void
BasicEffect::setAmbientLight(const AmbientLightFX& value)
{
  ambientLight_->SetRawValue(&value, 0, sizeof(value));
}

void
BasicEffect::setDirectionalLight(const DirectionalLightFX& value)
{
  directionalLight_->SetRawValue(&value, 0, sizeof(value));
}

void
BasicEffect::setPointLight(const PointLightFX& value)
{
  pointLight_->SetRawValue(&value, 0, sizeof(value));
}

void
BasicEffect::setSpotLight(const SpotLightFX& value)
{
  spotLight_->SetRawValue(&value, 0, sizeof(value));
}

void
BasicEffect::setEyePosW(FXMVECTOR value)
{
...
void
BasicEffect::setViewProjection(FXMMATRIX value)
{
  viewProjection_->SetMatrix(reinterpret_cast<const float*>(&(value)));
}

void
BasicEffect::setTextureTransform(FXMMATRIX value)
{
  textureTransform_->SetMatrix(reinterpret_cast<const float*>(&(value)));
}

void
BasicEffect::setDiffuseTex(ID3D11ShaderResourceView* value)
{
...
void
BasicEffect::setNormalTex(ID3D11ShaderResourceView* value)
{
  if (value) {
...
void
BasicEffect::setMaterial(const MaterialFX& material)
{
  material_->SetRawValue(&material, 0, sizeof(material));
}

void
BasicEffect::setFog(const EffectFog& value)
{
