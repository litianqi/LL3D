...
LightComponent::LightComponent(const Transform& transform,
...
void
LightComponent::writeToEffect() const
{
  if (type_ == Ambient) {
    s_effect->setAmbientLight(AmbientLightFX(light_.ambient));
  } else if (type_ == Directional) {
    s_effect->setDirectionalLight(
      DirectionalLightFX(light_.directional, transform_));
  } else if (type_ == Point) {
...
LightComponent::lightType() const
{
  return type_;
}
...
LightComponent::Light::operator=(const Light& rhs) noexcept
{
  Light tmp(rhs);
  std::swap(*this, tmp);
  return *this;
