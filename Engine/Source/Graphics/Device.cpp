  ...
  : enable4xMsaa_(config["enable_4x_msaa"].bool_value())
{
  ...
  device_->CheckMultisampleQualityLevels(DXGI_FORMAT_R8G8B8A8_UNORM, 4,
                                         &msaaQuality_);
...
Device::onResize(IntSize2 windowSize)
{
  ...
  throwIfFailed(
    device_->CreateRenderTargetView(back_buffer, 0, &renderTargetView_));
  back_buffer->Release();
  ...
  throwIfFailed(
    device_->CreateTexture2D(&depthStencilDesc, 0, &depthStencilBuffer_));
  throwIfFailed(device_->CreateDepthStencilView(depthStencilBuffer_.Get(), 0,
                                                &depthStencilView_));
...
ID3D11DeviceContext*
Device::deviceContex() const
{
  return context_.Get();
}

IDXGISwapChain*
Device::swapChain() const
{
  return swapChain_.Get();
}

ID3D11RenderTargetView*
Device::renderTargetView() const
{
  return renderTargetView_.Get();
}

ID3D11DepthStencilView*
Device::depthStencilView() const
{
