...
namespace LL3D
{
  namespace Graphics
  {
    ...
    StaticCamera::StaticCamera(float fovY, float aspectRatio, float nearZ,
                               float farZ)
      : fovY_(fovY), aspectRatio_(aspectRatio), nearZ_(nearZ), farZ_(farZ)
    {
    ...
    {
      proj_ = Math::Matrix::CreatePerspectiveFieldOfView(fovY_, aspectRatio_,
                                                         static_cast<float>(nearZ_),
                                                         static_cast<float>(farZ_));
    }

    const Math::Matrix &StaticCamera::projMaxtrix() const { return proj_; }
      ...
      transform_(transform)
    {
      ...
      ASSERT(transform.forwardVec().Cross(transform.upVec()) !=
             Math::Vector3::Zero);
    }

    const Math::Matrix &Camera::viewMatrix() const { return view_; }

    const Math::Matrix &Camera::viewProjMatrix() const { return viewProj_; }

    DirectX::BoundingFrustum Camera::frustum() const { return frustum_; }
