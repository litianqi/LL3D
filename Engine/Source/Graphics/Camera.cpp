...
void
StaticCamera::setAspectRatio(float aspectRatio)
{
  aspectRatio_ = aspectRatio;
}
...
StaticCamera::update()
{
  proj_ = Math::Matrix::CreatePerspectiveFieldOfView(
    fovY_, aspectRatio_, static_cast<float>(nearZ_), static_cast<float>(farZ_));
}

const Math::Matrix&
StaticCamera::projMaxtrix() const
{
  return proj_;
}
