...
void
createOpaque(ID3D11Device* device)
{
  // TODO
}

void
createAlphaBlend(ID3D11Device* device)
{
  ...
  throwIfFailed(device->CreateBlendState(&desc, &s_alphaBlend));
}

void
createSubstract(ID3D11Device* device)
{
  D3D11_BLEND_DESC desc;
  desc.AlphaToCoverageEnable = true;
  desc.IndependentBlendEnable = false;
  ...
  throwIfFailed(device->CreateBlendState(&desc, &s_substract));
}

void
CreateMultiply(ID3D11Device* device)
{
  D3D11_BLEND_DESC desc;
  desc.AlphaToCoverageEnable = true;
  desc.IndependentBlendEnable = false;
  ...
  throwIfFailed(device->CreateBlendState(&desc, &s_multiply));
}

void
CreateMakrMirror(ID3D11Device* device)
{
  ...
  throwIfFailed(device->CreateDepthStencilState(&desc, &s_markMirror));
}

void
CreateRenderReflection(ID3D11Device* device)
{
  auto desc = D3D11_DEPTH_STENCIL_DESC();
  desc.DepthEnable = true;                          // default
  ...
  throwIfFailed(device->CreateDepthStencilState(&desc, &s_renderReflection));
}

void
CreateShadow(ID3D11Device* device)
{
  auto desc = D3D11_DEPTH_STENCIL_DESC();
  desc.DepthEnable = true;                          // default
  desc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL; // default
  ...
  throwIfFailed(device->CreateDepthStencilState(&desc, &s_shadow));
}

void
createDepthLessEqual(ID3D11Device* device)
{
  auto desc = D3D11_DEPTH_STENCIL_DESC();
  desc.DepthEnable = true;                          // default
  desc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL; // default
  ...
  throwIfFailed(device->CreateDepthStencilState(&desc, &s_depthLessEqual));
}

void
CreateCullNone(ID3D11Device* device)
{
  ...
  throwIfFailed(device->CreateRasterizerState(&desc, &s_cullNone));
}

void
CreateCullClockwise(ID3D11Device* device)
{
  auto desc = D3D11_RASTERIZER_DESC();
  desc.FillMode = D3D11_FILL_SOLID;
  ...
  throwIfFailed(device->CreateRasterizerState(&desc, &s_cullClockwise));
}

void
CreateCullCounterClockwise(ID3D11Device* device)
{
  auto desc = D3D11_RASTERIZER_DESC();
  desc.FillMode = D3D11_FILL_SOLID;
  desc.CullMode = D3D11_CULL_BACK;
  ...
  throwIfFailed(device->CreateRasterizerState(&desc, &s_cullCounterClockwise));
}

void
createWireframe(ID3D11Device* device)
{
  auto desc = D3D11_RASTERIZER_DESC();
  ...
  throwIfFailed(device->CreateRasterizerState(&desc, &s_wireframe));
}
...
void
initialize(ID3D11Device* device)
{
  createOpaque(device);
