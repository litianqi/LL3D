...
float3 ApplyDiffuseAlbedo(float3 diffuseAlbedo, float3 diffuseLight)
{
  ...
  float3 viewDir)  // World-space vector from eye to point
{
  ...
  float3 specular = ApplySpecularAlbedoAndCone(mat.specular, mat.shininess, lightSpecular,
    lightDir, pointNormal, viewDir);
  ...
  float3 viewDir)  // World-space vector from eye to point
{
  ...
  float spot_factor = pow(max(dot(light2point, light.direction), 0), light.innerConeAngle);
