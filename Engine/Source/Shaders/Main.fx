...
VS(VertexIn vin)
{
  ...
  float tess = saturate((kMaxTessDist - d) / (kMaxTessDist - kMinTessDist));
...
[domain("tri")][partitioning("fractional_odd")][outputtopology("triangle_cw")]
               [outputcontrolpoints(3)][patchconstantfunc("PatchHS")] VertexOut
                 HS(InputPatch<VertexOut, 3> vout, uint i
...
[domain("tri")] DomainOut
DS(PatchTess patchTess, float3 bary
   : SV_DomainLocation, const OutputPatch<VertexOut, 3> tri)
{
  DomainOut dout;
  ...
  dout.posWS =
    bary.x * tri[0].posWS + bary.y * tri[1].posWS + bary.z * tri[2].posWS;
  dout.posLS =
    bary.x * tri[0].posLS + bary.y * tri[1].posLS + bary.z * tri[2].posLS;
  dout.texcoord = bary.x * tri[0].texcoord + bary.y * tri[1].texcoord +
                  bary.z * tri[2].texcoord;
  dout.normalWS = bary.x * tri[0].normalWS + bary.y * tri[1].normalWS +
                  bary.z * tri[2].normalWS;
  // Interpolating normal can unnormalize it, so normalize it.
  dout.normalWS = normalize(dout.normalWS);
  dout.tangentWS = bary.x * tri[0].tangentWS + bary.y * tri[1].tangentWS +
                   bary.z * tri[2].tangentWS;
  dout.tangentWS = normalize(dout.tangentWS);
  dout.bitangentWS = bary.x * tri[0].bitangentWS + bary.y * tri[1].bitangentWS +
                     bary.z * tri[2].bitangentWS;
  ...
  // the next miplevel every kMipInterval units, and clamp the miplevel in
  // [0,6].
  float d = distance(dout.posWS, g_eyePosWS.xyz);
  float mipLevel = clamp((d - kMipInterval) / kMipInterval, 0.0f, 6.0f);
...
float4
PS(DomainOut pin, uniform bool use_alpha_clip)
  : SV_Target
{
  float4 result;
  ...
  float3 directional_contribution = ApplyDirectionalLight(
    g_material, g_directional_light, pin.normalWS, viewDir);
  float3 point_contribution = ApplyPointLight(
    g_material, g_point_light, pin.posWS.xyz, pin.normalWS, viewDir);
  float3 spot_contribution = ApplySpotLight(
    g_material, g_spot_light, pin.posWS.xyz, pin.normalWS, viewDir);
