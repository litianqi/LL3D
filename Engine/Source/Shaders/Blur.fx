...
cbuffer Settings
{
  float g_weights[2 * RADIUS + 1] = { 0.05f, 0.05f, 0.1f, 0.1f,  0.1f, 0.2f,
...
[numthreads(GROUP_THREAD_NUM, 1, 1)] void
CSHorizonalBlur(int3 group_thread_id
                : SV_GroupThreadID, int3 dispatch_id
                : SV_DispatchThreadID)
{
  // First, fill g_cache
...
[numthreads(1, GROUP_THREAD_NUM, 1)] void
CSVerticalBlur(int3 group_thread_id
               : SV_GroupThreadID, int3 dispatch_id
               : SV_DispatchThreadID)
{
  // First, fill g_cache
  ...
  if (group_thread_id.y < RADIUS) {
    ...
    g_cache[group_thread_id.y] =
      g_input[int2(dispatch_id.x, y)]; // TODO: this is different to book.
  }
  ...
  g_cache[group_thread_id.y + RADIUS] =
    g_input[min(dispatch_id.xy, g_input.Length.xy - int2(0, 1))]; // TODO: this
                                                                  // is
                                                                  // different
                                                                  // to book.
