...
GameObject::GameObject(const std::string& name) noexcept : name_(name)
{
  auto transform = make_unique<Transform>(nullptr);
  addComponent(std::move(transform));
}
...
void
GameObject::setName(const std::string& value)
{
  name_ = value;
}

void
GameObject::setTag(const std::string& value)
{
  tag_ = value;
}
...
{
  return const_cast<Transform&>(
    const_cast<const GameObject*>(this)->transform());
}
