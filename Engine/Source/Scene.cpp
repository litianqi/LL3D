...
// We render models in this order: opaque -> shadow -> mark mirror ->
// reflection.
// And after mark mirror, though pixels of shadow intersected with mirror will
// be
// replaced, depth buffer will stay unchanged.
// There is a possibilty that opaque, shadow and reflection have a same
// position.
...
bool
SortBasedOnDistanceToCamera(Math::Vector3& camera_pos,
                            const RenderableMesh& lhs,
...
//
void
RenderPlanarShadow(Transform transform, const Graphics::MeshRender& mesh,
                   const std::vector<const GameObject*>& lights)
{
  ...
  for (const auto& light : lights) {
    ...
    Transform::writeToEffect(world * shadow *
                             Math::Matrix::CreateTranslation(ShadowOffset));
...
void
Scene::add(std::unique_ptr<GameObject> object)
{
...
Scene::render()
{
  ...
  // Make sure pixels far away that depth values equals 1 (NDC) will be
  // rendered.
  // (e.g. skybox)
  ...
  s_graphicsDevice->deviceContex()->OMSetBlendState(nullptr, nullptr,
                                                    0xffffffff);
  ...
  for (const auto& mirror : mirrors()) {
    ...
    const auto plane =
      Math::Plane(mirror.first.position(), mirror.first.upVec());
    ...
    s_graphicsDevice->deviceContex()->OMSetDepthStencilState(nullptr, 1);
...
Scene::calculatePicking()
{
  ...
  if (camera() != nullptr) {
    if (Input::Mouse::isHeldingDown(Input::Mouse::kLeft)) {
      ...
      const auto rayVS =
        Math::Ray(Math::Vector3::Zero, Math::Vector3(xVS, yVS, 1.f));
      auto rayWS = rayVS;
      ...
      rayWS.direction =
        Math::Vector3::TransformNormal(rayWS.direction, view.Invert());
      ...
      for (const auto& object : RecursiveSceneIterator(*this)) {
        ...
        if (model) {
          ...
          rayLS.position =
            Math::Vector3::Transform(rayLS.position, world.Invert());
          rayLS.direction =
            Math::Vector3::TransformNormal(rayLS.direction, world.Invert());
            ...
            for (const auto& meshRender : *model) {
              ...
              for (int i = 2; i < mesh.indices.size(); i += 3) {
                if (rayLS.Intersects(
                      ...
                      mesh.vertices[mesh.indices[i]].position, tmp)) {
