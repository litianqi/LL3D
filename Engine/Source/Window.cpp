...
Window::Window(json11::Json config)
{
  ...
  // Create window
  handle_ = CreateWindowEx(0, L"LL3DWndClass", L"LL3D", WS_OVERLAPPEDWINDOW,
  ...
  /* handle_ = CreateWindow(L"LL3DWndClass", L"aa",
     WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 800, 400, 0, 0,
...
void
Window::setClientRect(IntRectangle rect)
{
  RECT r{ rect.GetLeft(), rect.GetTop(), rect.GetRight(), rect.GetBottom() };
  WA(AdjustWindowRect(&r, WS_OVERLAPPEDWINDOW, false));
}

void
Window::onResize(std::function<void(void)> callback)
{
...
LRESULT
Window::msgProc(HWND handle, UINT msg, WPARAM wparam, LPARAM lparam)
{
  Window* self =
