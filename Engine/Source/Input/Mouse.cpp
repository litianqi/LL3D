...
Mouse::processMessage(UINT message, WPARAM wparam, LPARAM lparam)
{
  ...
  auto track = TRACKMOUSEEVENT{ sizeof(TRACKMOUSEEVENT), TME_HOVER | TME_LEAVE,
                                s_hwnd, HOVER_DEFAULT };
...
bool
Mouse::isHeldingDown(Button button)
{
  return s_heldingDown[button];
}

bool
Mouse::isPressed(Button button)
{
  return s_pressed[button];
}

bool
Mouse::isReleased(Button button)
{
